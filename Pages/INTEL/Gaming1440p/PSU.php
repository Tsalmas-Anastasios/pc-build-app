...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-INTEL/Category: Hard Gaming in 1440p/PSU select</TITLE>
...
<SECTION>
	...
	if ($_POST["brandModelCPU"] != "") {
		...
		if ($notSearch == 0) {
			...
			$sql = "SELECT * FROM PSU WHERE Category='Hard Gaming in 1440p' AND Description LIKE '%$search_expression%'";
			$result = $conn->query($sql);
			if ($result->num_rows <=0 ) {
				$sql = "SELECT * FROM PSU WHERE Category='Hard Gaming in 1440p' AND Description LIKE '%$search_post%'";
				$result = $conn->query($sql);
				if ($result->num_rows <=0 ) {
					$sql = "SELECT * FROM PSU WHERE Category='Hard Gaming in 1440p'$keywords";
					$result = $conn->query($sql);
					if ($result->num_rows <= 0) {
						$sql = "SELECT * FROM PSU WHERE Category='Hard Gaming in 1440p'";
						$result = $conn->query($sql);
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				if ($row["watt"] >= $intvalPsuGpu) {
					...
					echo '</div>';
				  echo '</div>';
					echo '<input type="hidden" id="categoryPSU" name="categoryPSU" value="Hard Gaming in 1440p">';
				...
				if ($metra == "0") {
					echo '<div class="no-results-search">
    	<img src="http://pc-build-webapp.000webhostapp.com/Pictures/pc-builder/no-results-search.png" alt="No results" class="no-results-search-img">
        <form action="../../../../Pages/INTEL/Gaming1440p/GPU" method="post" name="NothingForm">
          <center><div class="no-results-search-message">
