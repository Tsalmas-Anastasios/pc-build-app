...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-INTEL/Category: Moderate Gaming/Ram select</TITLE>
...
<SECTION>
    ...
    if ($_POST["brandModelCPU"] != "") {
        ...
        if ($notSearch == 0) {
          ...
          $sql = "SELECT * FROM RAM WHERE Category='Moderate Gaming' AND Description LIKE '%$search_expression%'";
          $result = $conn->query($sql);
          if ($result->num_rows <=0 ) {
            $sql = "SELECT * FROM RAM WHERE Category='Moderate Gaming' AND Description LIKE '%$search_post%'";
            $result = $conn->query($sql);
            if ($result->num_rows <=0 ) {
              $sql = "SELECT * FROM RAM WHERE Category='Moderate Gaming'$keywords";
              $result = $conn->query($sql);
              if ($result->num_rows <= 0) {
                $sql = "SELECT * FROM RAM WHERE Category='Moderate Gaming'";
                $result = $conn->query($sql);
          ...
          if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                  ...
                  if ($ramSpMod == $row["speed"]) {
                    ...
                    //DATA FROM BLUETOOTH ADAPTERS
                      ...
                      echo '<button type="button" class="productCard-Button"></font></button>
                              ...
                              echo '</div>';
                      echo '</div>';
                      echo '<input type="hidden" id="categoryRAM" name="categoryRAM" value="Moderate Gaming">';
              ...
              if ($metra == "0") {
                ...
                echo "<br><br><br>";
                echo '<form action="../../../../Pages/INTEL/ModerateGaming/Motherboard" method="post" name="NothingForm">';
