...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-INTEL/Category: Simple Edit/PSU select</TITLE>
...
<SECTION>
	...
	if ($_POST["brandModelCPU"] != "") {
		...
		if ($notSearch == 0) {
			...
			$sql = "SELECT * FROM PSU WHERE Category='Simple Edit' AND Description LIKE '%$search_expression%'";
			$result = $conn->query($sql);
			if ($result->num_rows <=0 ) {
				$sql = "SELECT * FROM PSU WHERE Category='Simple Edit' AND Description LIKE '%$search_post%'";
				$result = $conn->query($sql);
				if ($result->num_rows <=0 ) {
					$sql = "SELECT * FROM PSU WHERE Category='Simple Edit'$keywords";
					$result = $conn->query($sql);
					if ($result->num_rows <= 0) {
						$sql = "SELECT * FROM PSU WHERE Category='Simple Edit'";
						$result = $conn->query($sql);
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				if ($row["watt"] >= $intvalPsuGpu) {
					...
					echo '</div>';
				  echo '</div>';
					echo '<input type="hidden" id="categoryPSU" name="categoryPSU" value="Simple Edit">';
				...
				if ($metra == "0") {
					echo '<div class="no-results-search">
    	<img src="http://pc-build-webapp.000webhostapp.com/Pictures/pc-builder/no-results-search.png" alt="No results" class="no-results-search-img">
        <form action="../../../../Pages/INTEL/SimpleEdit/GPU" method="post" name="NothingForm">
          <center><div class="no-results-search-message">
