...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-Ready PC Build-Category:<?php echo $_POST["categoryCPU"]; ?>/Headset select</TITLE>
...
<?php
    ...
    if ($conn->connect_error) {
          ...
          <div class="modal-dialog" style="width: 700px;">
            ...
            <section class="modal-content">
                ...
                //TAKE DATA FROM Database AND COUNT VARIABLES --- START
                  $issetStep = 0;
                  if (isset($_POST["connSubmit"]) or isset($_POST["wireless"]) or isset($_POST["surround"]) or isset($_POST["mic"]) or isset($_POST["rgb"]) or isset($_POST["overEar"]) or isset($_POST["headBand"]) or isset($_POST["inEar"]) or isset($_POST["onEar"]) or isset($_POST["earHook"]) or isset($_POST["neck"])) {
                    $keywords = "";
                    $key = "";
                    if (isset($_POST["connSubmit"])) {
                      $key = $_POST["connSubmit"];
                      $keywords = " connection='$key'";
                    }
                    ...
                    $sql = "SELECT * FROM Gaming_Headset WHERE$keywords";
                  } else {
                    $sql = "SELECT * FROM Gaming_Headset";
                  }
                  $result = $conn->query($sql);
                  if ($result->num_rows > 0) {
                    //Variables for connection way --- START
                    $usb = 0;
                    $jack = 0;
                    $bluetooth = 0;
                    //Variables for connection way --- END
                    ...
                    while($row = $result->fetch_assoc()) {
                      //CONNECTION WAY---------------------START
                      if ($row["connection"] == "USB") {
                        $usb++;
                      } elseif ($row["connection"] == "3.5mm Jack") {
                        $jack++;
                      } elseif ($row["connection"] == "Bluetooth") {
                        $bluetooth++;
                      }
                      //CONNECTION WAY--------------------END
                      //FEATURES---------------------START
                      ...
                      foreach($explode as $explode) {
                        if ($explode == "Wireless") {
                          $wireless++;
                        } elseif ($explode == "Surround Sound") {
                          $surround++;
                        } elseif ($explode == "Sound isolation microphone") {
                          $mic++;
                        } elseif ($explode == "RGB") {
                          $rgb++;
                        }
                      ...
                      //FEATURES---------------------END
                      //TYPE---------------------START
                      $explode = explode(',', $row["type"]);
                      foreach($explode as $explode) {
                        if ($explode == "Over Ear") {
                          $over_ear++;
                        } elseif ($explode == "HeadBand") {
                          $head_band++;
                        } elseif ($explode == "In Ear") {
                          $in_ear++;
                        } elseif ($explode == "On Ear") {
                          $on_ear++;
                        } elseif ($explode == "Earhook") {
                          $earhook++;
                        } elseif ($explode == "Neckband") {
                          $neck++;
                        }
                      }
                      //TYPE---------------------END
                  ...
                  $not_isset = 0;
                  //ALL ISSETS FROM CHECKBOXES NUMBER: 10
                  if (!isset($_POST["wireless"])) {
                    $not_isset++;
                  }
                  if (!isset($_POST["surround"])) {
                  ...
                  //IF ONLY connSubmit isset---START
                    if (isset($_POST["connSubmit"]) and $not_isset == 10) {
                      //Variables for connection way --- START
                      $usb = 0;
                      $jack = 0;
                      $bluetooth = 0;
                  ...
                  //IF ONLY connSubmit isset---END
                  //IF ISSET connSubmit and an input from features---START
                    if (isset($_POST["connSubmit"]) and $not_isset < 10) {
                      $keywords = "";
                      $key = "";
                      if (isset($_POST["wireless"])) {
                      ...
                      if (isset($_POST["overEar"])) {
                        $key = $_POST["overEar"];
                        if ($keywords == "") {
                          $keywords = " type LIKE '%$key%'";
                        } else {
                          $keywords .= " AND type LIKE '%$key'";
                        }
                      }
                      if (isset($_POST["headBand"])) {
                        $key = $_POST["headBand"];
                  ...
                  //IF ISSET connSubmit and an input from features---END
                //TAKE DATA FROM Database AND COUNT VARIABLES --- END

                //CREATE OPTION BOXES --- START
                  //CONNECTION WAY-----------START
                  if ($usb > 0 or $jack > 0 or $bluetooth > 0) {
                    echo '<p class="typesOfDisksInCategorySSD">
                      ...
                      if ($jack > 0) {
                        echo '<label class="containerRadioButton">3.5mm Jack ('. $jack. ')
                          <input type="radio" name="connSubmit" id="Jack" value="3.5mm Jack" onclick="footerModal();submitForFilters()"> <!--checked="checked"-->
                          <span class="checkmarkRadioButton"></span>
                        </label>&nbsp;&nbsp;&nbsp;';
                      } else {
                        echo '<label class="containerRadioButton"><font color="#b3b3b3">3.5mm Jack (0)</font>
                          <input type="radio" name="connSubmit" id="Jack" value="3.5mm Jack" disabled> <!--checked="checked"-->
                          <span class="checkmarkRadioButton"></span>
                        </label>&nbsp;&nbsp;&nbsp;';
                      }
                      if ($bluetooth > 0) {
                    ...
                    if ($wireless > 0 or $surround > 0 or $mic > 0 or $rgb > 0 or $over_ear > 0 or $head_band > 0 or $in_ear > 0 or $on_ear > 0 or $earhook > 0 or $neck > 0) {
                      echo '<br><hr class="hrModalBox"><br>';
                    }
                  }
                  //CONNECTION WAY-----------END
                  //FEATURES-----------START
                  if ($wireless > 0 or $surround > 0 or $mic > 0 or $rgb > 0) {
                    echo '<p class="typesOfDisksInCategorySSD">
                      <b>Features</b>
                    </p><br>';
                      if ($wireless > 0) {
                        echo '<label class="containerCheckBox">Wireless ('. $wireless.')
                          <input type="checkbox" class="CheckBox" id="wireless" name="wireless" value="Wireless" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Wireless (0)</font>
                          <input type="checkbox" class="CheckBox" id="wireless" name="wireless" value="Wireless" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($surround > 0) {
                        echo '<label class="containerCheckBox">Surround Sound ('. $surround.')
                          <input type="checkbox" class="CheckBox" id="surround" name="surround" value="Surround Sound" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Surround Sound (0)</font>
                          <input type="checkbox" class="CheckBox" id="surround" name="surround" value="Surround Sound" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($mic > 0) {
                        echo '<label class="containerCheckBox">Sound isolation microphone ('. $mic.')
                          <input type="checkbox" class="CheckBox" id="mic" name="mic" value="Sound isolation microphone" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Sound isolation microphone (0)</font>
                          <input type="checkbox" class="CheckBox" id="mic" name="mic" value="Sound isolation microphone" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($rgb > 0) {
                        echo '<label class="containerCheckBox">RGB ('. $rgb.')
                          <input type="checkbox" class="CheckBox" id="rgb" name="rgb" value="RGB" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">RGB (0)</font>
                          <input type="checkbox" class="CheckBox" id="rgb" name="rgb" value="RGB" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                    if ($over_ear > 0 or $head_band > 0 or $in_ear > 0 or $on_ear > 0 or $earhook > 0 or $neck > 0) {
                      echo '<br><hr class="hrModalBox"><br>';
                    }
                  }
                  //FEATURES-----------END
                  ...
                  if ($over_ear > 0 or $head_band > 0 or $in_ear > 0 or $on_ear > 0 or $earhook > 0 or $neck > 0) {
                    echo '<p class="typesOfDisksInCategorySSD">
                      ...
                      if ($over_ear > 0) {
                        echo '<label class="containerCheckBox">Over Ear ('. $over_ear.')
                          <input type="checkbox" class="CheckBox" id="overEar" name="overEar" value="Over Ear" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Over Ear (0)</font>
                          <input type="checkbox" class="CheckBox" id="overEar" name="overEar" value="Over Ear" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($head_band > 0) {
                        echo '<label class="containerCheckBox">HeadBand ('. $head_band.')
                          <input type="checkbox" class="CheckBox" id="headBand" name="headBand" value="HeadBand" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">HeadBand (0)</font>
                          <input type="checkbox" class="CheckBox" id="headBand" name="headBand" value="HeadBand" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($in_ear > 0) {
                        echo '<label class="containerCheckBox">In Ear ('. $in_ear.')
                          <input type="checkbox" class="CheckBox" id="inEar" name="inEar" value="In Ear" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">In Ear (0)</font>
                          <input type="checkbox" class="CheckBox" id="inEar" name="inEar" value="In Ear" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($on_ear > 0) {
                        echo '<label class="containerCheckBox">On Ear ('. $on_ear.')
                          <input type="checkbox" class="CheckBox" id="onEar" name="onEar" value="On Ear" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">On Ear (0)</font>
                          <input type="checkbox" class="CheckBox" id="onEar" name="onEar" value="On Ear" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($earhook > 0) {
                        echo '<label class="containerCheckBox">Earhook ('. $earhook.')
                          <input type="checkbox" class="CheckBox" id="earHook" name="earHook" value="Earhook" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Earhook (0)</font>
                          <input type="checkbox" class="CheckBox" id="earHook" name="earHook" value="Earhook" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                      if ($neck > 0) {
                        echo '<label class="containerCheckBox">Neckband ('. $neck.')
                          <input type="checkbox" class="CheckBox" id="neck" name="neck" value="Neckband" onclick="footerModal();submitForFilters()">
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      } else {
                        echo '<label class="containerCheckBox"><font color="#b3b3b3">Neckband (0)</font>
                          <input type="checkbox" class="CheckBox" id="neck" name="neck" value="Neckband" disabled>
                          <span class="checkmarkCheckBox"></span>
                        </label>';
                      }
                  }
              ...
              <!--SCRIPT FOR CLEAR FILTERS-->
                <script>
                  function clearFiltersFunction() {';
                    ...
                    //CONNECTION WAY----------------START
                    if (isset($_POST["connSubmit"])) {
                      if ($_POST["connSubmit"] == "3.5mm Jack") {
                        $key_javascript = "Jack";
                      } else {
                        $key_javascript = $_POST["connSubmit"];
                      }
                    ...
                    //FEATURES-----------START
                      //WIRELESS---START
                      if (isset($_POST["wireless"])) {
                        $key_javascript = "wireless";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //WIRELESS---END
                      //SURROUND SOUND---START
                      if (isset($_POST["surround"])) {
                        $key_javascript = "surround";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //SURROUND SOUND---END
                      //SOUND ISOLATION MICROPHONE---START
                      if (isset($_POST["mic"])) {
                        $key_javascript = "mic";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //SOUND ISOLATION MICROPHONE---END
                      //RGB---START
                      if (isset($_POST["rgb"])) {
                        $key_javascript = "rgb";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                    ...
                    //TYPE----------START
                      //OVER EAR---START
                      if (isset($_POST["overEar"])) {
                        $key_javascript = "overEar";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //OVER EAR---END
                      //HeadBand---START
                      if (isset($_POST["headBand"])) {
                        $key_javascript = "headBand";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //HeadBand---END
                      //IN EAR---START
                      if (isset($_POST["inEar"])) {
                        $key_javascript = "inEar";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //IN EAR---END
                      //ON EAR---START
                      if (isset($_POST["onEar"])) {
                        $key_javascript = "onEar";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //ON EAR---END
                      //EARHOOK---START
                      if (isset($_POST["earHook"])) {
                        $key_javascript = "earHook";
                        echo 'document.getElementById("'. $key_javascript. '").checked = false;';
                      }
                      //EARHOOK---END
                      //NECKBAND---START
        ...
        if(isset($_POST["searchBar"])) {
          ...
          if ($searcher == 1) {
            if ($notSearch == 0) {
              ...
              $sql = "SELECT * FROM Gaming_Headset WHERE Description LIKE '%$search_expression%'";
              $result = $conn->query($sql);
              if ($result->num_rows <= 0) {
                $sql = "SELECT * FROM Gaming_Headset WHERE Description LIKE '%$search_post%'";
                $result = $conn->query($sql);
                if ($result->num_rows <=0 ) {
                  $sql = "SELECT * FROM Gaming_Headset WHERE $keywords";
                  $result = $conn->query($sql);
            ...
            if ($result->num_rows > 0) {
              ...
              while($row=$result->fetch_assoc()) {
                ...
                echo '<form action="../../../Pages/PCBuilderPCBuild" method="post" id="form'. $onoma_for_title./*id for form in product card*/ '" name="form'. $onoma_for_title./*id for form in product card*/'">';
                    ...
                    echo '<input type="hidden" id="HEADERPiece" name="HEADERPiece" value="Headset">';
                ...
                echo '<input type="hidden" id="brandModelHEADER" name="brandModelHEADER" value="'. $row["Brand"]. ' '. $row["Model"]. '">';
                echo '<b><font size="2px">Product Code: '. $row["ProductCode"]. '</font></b>';  
                echo '<input type="hidden" id="ProductCodeHEADER" name="ProductCodeHEADER" value="'. $row["ProductCode"]. '">';
                echo '<br><br><br><br><br>';
                echo '<button type="button" class="productCard-Button"></font></button>
                  <div class="specs">';
                    ...
                    echo '<center><font color="black"><table class="tableSpecs">
                      ...
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Cable length</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["cable"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Connection</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["connection"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                      ...
                      <tr class="tableSpecs">
                        <th class="rowsOnSpecs">Technical chaaracteristics</th>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Headphone frequency</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["hz"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Headphone sensitivity</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["ey"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Headphone resistance</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["an"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Microphone frequency</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["hz_mic"]. '</td>
                      </tr>
                      <tr class="rowsOnSpecsTR">
                        <td class="rowsOnSpecs">Microphone sensitivity</td>
                        <td class="rowsOnSpecs">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                        <td class="rowsOnSpecs">'. $row["ey_mic"]. '</td>
                      </tr>
                    </table></font></center>';
                  echo '</div>';
                echo '</div>';
                echo '<input type="hidden" id="categoryHEADER" name="categoryHEADER" value="Gaming Headset">';
                //INFO OF PRODUCTS-END
                //PRICE OF PRODUCTS
                echo '<div class="productCardpreview">';
                if ($ekptwshPrice == $PriceMod) {
                  echo '<h2 align="right">'. $row["Price"]. '€</h2>';
                  echo '<input type="hidden" id="PriceHEADER" name="PriceHEADER" value="'. $row["Price"]. '">';
                  echo '<input type="hidden" id="EkptwshHEADER" name="EkptwshHEADER" value="">';
                  ...
                  echo '<input type="hidden" id="PriceHEADER" name="PriceHEADER" value="'. $row["PriceEkptwsh"]. '">';
                  echo '<input type="hidden" id="EkptwshHEADER" name="EkptwshHEADER" value="'. $row["Price"]. '">';
                }
                if ($brINbutton = 0) {
                ...
                echo '<form action="../../../Pages/PCBuilderPCBuild" method="post" id="form'. $onoma_for_title./*id for form in product card*/ '" name="form'. $onoma_for_title./*id for form in product card*/'">';
                  ...
                  echo '<input type="hidden" id="pieceHEADER" name="HEADERPiece" value="Headset">';
                  ...
                  echo '<input type="hidden" id="brandModelHEADER" name="brandModelHEADER" value="'. $row["Brand"]. ' '. $row["Model"]. '">';
                  echo '<input type="hidden" id="ProductCodeHEADER" name="ProductCodeHEADER" value="'. $row["ProductCode"]. '">';
                  echo '<input type="hidden" id="categoryHEADER" name="categoryHEADER" value="Gaming Headset"><br><br>';
                  if ($ekptwshPrice == $PriceMod) {
                  ...
                  echo '<br><center><button type="button" class="collapsibleMin">Specs</button>';
                    echo '<div class="contentMin">';
                        echo '<table class="tableSpecsMin">
                            ...
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Cable length</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["cable"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                                ...
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["connection"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                            ...
                            <!--ONE SPEC-->
                            <tr>
                              <th class="KEFALIDAmin">Technical characteristics</th>
                            </tr>
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Headphones frequency</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["hz"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Headphones sensitivity</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["ey"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Headphones resistance</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["an"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Microphone frequency</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["hz_mic"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE SPEC-->
                                <tr>
                                <td class="NameOfSpecsMin">Microphone sensitivity</td>
                                </tr>
                                <tr>
                                <td class="FromDatabaseSpecsMin">'. $row["ey_mic"]. '</td>
                                </tr>
                            <!--ONE SPEC-->
                            <!--ONE CATEGORY OF SPECS-->
                        </table>';
        ...
        if ($clearStep == 0 and (isset($_POST["connSubmit"]) or isset($_POST["wireless"]) or isset($_POST["surround"]) or isset($_POST["mic"]) or isset($_POST["rgb"]) or isset($_POST["overEar"]) or isset($_POST["headBand"]) or isset($_POST["inEar"]) or isset($_POST["onEar"]) or isset($_POST["earHook"]) or isset($_POST["neck"])) and $searcher == 0) {
          $keywords = "";
          $key = "";
          $key_javascript = "";
		...
		<center><div class="no-results-search-message">
          ...
          if ($result->num_rows > 0) {
            ...
            echo '<br><br><center><b><font color="#b32400"><font size="5">We do not have any Headset right now!</font></font></b></center><br><br>';
          }
        }
        //RESULTS
      $conn->close();
