...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-Ready PC Build-Category:<?php echo $_POST["categoryCPU"]; ?>/More Ram select</TITLE>
    ...
    * {
    ...
    if ($_POST["brandModelCPU"] != "") {
        ...
        if ($notSearch == 0) {
          ...
          $sql = "SELECT * FROM RAM WHERE Category='$category' AND Description LIKE '%$search_expression%'";
          $result = $conn->query($sql);
          if ($result->num_rows <=0 ) {
            $sql = "SELECT * FROM RAM WHERE Category='$category' AND Description LIKE '%$search_post%'";
            $result = $conn->query($sql);
            if ($result->num_rows <=0 ) {
              $sql = "SELECT * FROM RAM WHERE Category='$category'$keywords";
              $result = $conn->query($sql);
              if ($result->num_rows <= 0) {
                $sql = "SELECT * FROM RAM WHERE Category='$category'";
                $result = $conn->query($sql);
                ...
                foreach($ramSpMod as $ramSpMod) {
                  if ($ramSpMod == $row["speed"]) {
                    ...
                    echo '<form action="../../../Pages/PCBuilderPCBuild" method="post" id="form'. $onoma_for_title./*id for form in product card*/ '" name="form'. $onoma_for_title./*id for form in product card*/'">';
                      ...
                      echo '<input type="hidden" id="RAM2Piece" name="RAM2Piece" value="More Ram">';
                      ...
                      echo '<input type="hidden" id="brandModelRAM2" name="brandModelRAM2" value="'. $row["Brand"]. ' '. $row["Model"]. '">';
                      echo '<b><font size="2px">Product Code: '. $row["ProductCode"]. '</font></b>';  
                      echo '<input type="hidden" id="ProductCodeRAM2" name="ProductCodeRAM2" value="'. $row["ProductCode"]. '">';
                      echo '<br><br><br><br><br>';
                      echo '<button type="button" class="productCard-Button"></font></button>
                              ...
                              echo '</div>';
                      echo '</div>';
                      echo '<input type="hidden" id="categoryRAM2" name="categoryRAM2" value="'. $category. '">';
                      //INFO OF PRODUCTS-END
                      //PRICE OF PRODUCTS
                      echo '<div class="productCardpreview">';
                      if ($ekptwshPrice == $PriceMod) {
                        echo '<h2 align="right">'. $row["Price"]. '€</h2>';
                        echo '<input type="hidden" id="PriceRAM2" name="PriceRAM2" value="'. $row["Price"]. '">';
                        echo '<input type="hidden" id="EkptwshRAM2" name="EkptwshRAM2" value="">';
                        ...
                        echo '<input type="hidden" id="PriceRAM2" name="PriceRAM2" value="'. $row["PriceEkptwsh"]. '">';
                        echo '<input type="hidden" id="EkptwshRAM2" name="EkptwshRAM2" value="'. $row["Price"]. '">';
                      }
                      if ($brINbutton = 0) {
