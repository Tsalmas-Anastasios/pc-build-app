...
<html lang="en">
	...
	<TITLE>PC Build App-PC Builder-Ready PC Build-Category:<?php echo $_POST["categoryCPU"]; ?>/LED for PC Case select</TITLE>
...
<?php
    ...
    if ($notSearch == 0) {
        ...
        $sql = "SELECT * FROM Led_PC WHERE Category='$category' AND Description LIKE '%$search_expression%'";
        $result = $conn->query($sql);
        if ($result->num_rows <=0 ) {
            $sql = "SELECT * FROM Led_PC WHERE Category='$category' AND Description LIKE '%$search_post%'";
            $result = $conn->query($sql);
            if ($result->num_rows <=0 ) {
                $sql = "SELECT * FROM Led_PC WHERE Category='$category'$keywords";
                $result = $conn->query($sql);
                if ($result->num_rows <= 0) {
                    $sql = "SELECT * FROM Led_PC WHERE Category='$category'";
                    $result = $conn->query($sql);
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo '<form action="../../../Pages/PCBuilderPCBuild" method="post" id="form'. $onoma_for_title./*id for form in product card*/ '" name="form'. $onoma_for_title./*id for form in product card*/'">';
                ...
                echo '<input type="hidden" id="LEDPCPiece" name="LEDPCPiece" value="Led for Case">';
                ...
                echo '<input type="hidden" id="brandModelLEDPC" name="brandModelLEDPC" value="'. $row["Brand"]. ' '. $row["Model"]. '">';
                echo '<b><font size="2px">Product Code: '. $row["ProductCode"]. '</font></b>';  
                echo '<input type="hidden" id="ProductCodeLEDPC" name="ProductCodeLEDPC" value="'. $row["ProductCode"]. '">';
                echo '<br><br><br><br><br>';
                if ($row["Picture"] != NULL) {
                ...
                echo '</div>';
                echo '<input type="hidden" id="categoryLEDPC" name="categoryLEDPC" value="'. $row["Category"]. '">';
                //INFO OF PRODUCTS-END
                //PRICE OF PRODUCTS
                echo '<div class="productCardpreview">';
                if ($ekptwshPrice == $PriceMod) {
                echo '<h2 align="right">'. $row["Price"]. '€</h2>';
                echo '<input type="hidden" id="PriceLEDPC" name="PriceLEDPC" value="'. $row["Price"]. '">';
                echo '<input type="hidden" id="EkptwshLEDPC" name="EkptwshLEDPC" value="">';
                ...
                echo '<input type="hidden" id="PriceLEDPC" name="PriceLEDPC" value="'. $row["PriceEkptwsh"]. '">';
                echo '<input type="hidden" id="EkptwshLEDPC" name="EkptwshLEDPC" value="'. $row["Price"]. '">';
                }
                if ($brINbutton = 0) {
                ...
                echo '<div class="productCodeCard">Product Code: '. $row["ProductCode"]. '</div><br>';
                echo '<input type="hidden" id="brandModelLEDPC" name="brandModelLEDPC" value="'. $row["Brand"]. ' '. $row["Model"]. '">';
                echo '<input type="hidden" id="ProductCodeLEDPC" name="ProductCodeLEDPC" value="'. $row["ProductCode"]. '">';
                echo '<input type="hidden" id="categoryLEDPC" name="categoryLEDPC" value="'. $row["Category"]. '"><br>';
                if ($ekptwshPrice == $PriceMod) {
