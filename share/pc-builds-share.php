<?php
    ...
    if (isset($_POST["save"])) {
        if (isset($_SESSION["username"]) or isset($_SESSION["email"])) {
            ...
            $brand = $_POST["cpuBrand"];
            $category = $_POST["category"];
            $number = rand();
            $boolean = NULL;
            do {
                $count = 0;
                    $sql = "SELECT * FROM PC_Builds";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
            ...
            $sql = "INSERT INTO PC_Builds (username, email, cpu_brand, category, number) VALUES ('$username', '$email', '$brand', '$category', '$number')";
            if ($conn->query($sql) === TRUE) {
                //NOTHING OR JUST GO HIM/HER TO HIS/HER DASHBOARD!
                header("Location: ../sign-in/dashboard.php");
            } else {
...
<!--TopNav-->
    ...
    <div class="main">
        <?php
            $build = $_GET["pcbuild"];
            $sql = "SELECT * FROM PC_Builds WHERE number='$build'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                //WE FOUND THE PC BUILD AND NOW WE CAN DISPLAY IT
                $number_card = 0;
                while($row = $result->fetch_assoc()) {
                    $number_card++;
                    ?>
                                ...
                                <tr>
                                    ...
                                    <td class="other-td">
                                        ...
                                        <tr>
                                            ...
                                            <input type="hidden" name="cpuBrand" value="<?php echo $row["cpu_brand"]; ?>">
                                        </tr>
                                        <tr>
                                            <td><b>Category:</b></td>
                                        ...
                                        <div class="button-area">
                                            ...
                                            <button name="save" class="buy-it"><i class="fas fa-save"></i> Save it</button>
                                        </div>
                                    </td>
                                </tr>
                            </table>
                ...
                //WE DON'T HAVE ANY PC BUILD WITH THIS NUMBER-CODE
                //SHOULD DISPLAY AN ERROR MESSAGE
            ?>
                <div class="no-build-search">
                    <img src="../logo/pictureForTabs.png" alt="No results" class="no-build-search-img">
                    <center><div class="no-build-search-message">
                        There is no PC Build made by any user with this link or number! Made yours by click our PC Builder<br>
                        <a href="" style="text-decoration: none;color: #1a0033fa;">AMD</a>&nbsp;or&nbsp;
                        <a href="" style="text-decoration: none;color: #1a0033fa;">INTEL</a>
                    </div></center>
                </div>
            <?php
            }
